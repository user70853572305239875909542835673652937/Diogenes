...
router.get("/", async (req: Request, res: Response) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json(generateUsageResponse("Missing or invalid query parameters.", 400));
  }
  ...
  if (!isValidProvider(provider)) {
    return res
      .status(400)
      .json(
        generateUsageResponse(
          "Invalid provider. Allowed values: 'anilist', 'mal'.",
          400,
        ),
      );
  }
  ...
  if (!isValidContentType(type)) {
    return res
      ...
      .json(
        generateUsageResponse(
          "Invalid type. Allowed values: 'anime', 'manga'.",
          400,
        ),
      );
  }
  ...
  try {
    ...
    if (!data) {
      return res
        ...
        .json(
          generateUsageResponse(
            "Mappings not found for the given ID, provider, and type.",
            404,
          ),
        );
    }
