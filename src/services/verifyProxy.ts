...
async function verifyProxy(proxy: {
  ...
  try {
    ...
    console.log(
      `Proxy working: ${proxy.host}:${proxy.port} - Your IP: ${response.data.origin}`,
    );
    return true;
  } catch (error) {
 ...
 */
async function getRandomWorkingProxy(): Promise<{
  host: string;
  port: number;
} | null> {
  const proxies = JSON.parse(
    fs.readFileSync("./proxies.json", "utf8"),
  ) as string[];
...
(async () => {
  ...
  if (workingProxy) {
    console.log(
      `Found working proxy: ${workingProxy.host}:${workingProxy.port}`,
    );
  }
})();
